...
export function validateComponentManifest(manifest, root) {
  ...
  const replacesCoreFiles = (manifest.replacesCoreFiles ?? []).map((entry, index) =>
    safePath(entry, `replacesCoreFiles[${index}]`),
  );
  const persistentFiles = (manifest.persistentFiles ?? []).map((entry, index) =>
    safePath(entry, `persistentFiles[${index}]`),
  );
  ...
  if (new Set(replacesCoreFiles).size !== replacesCoreFiles.length) throw new Error("Duplicate replacesCoreFiles path");
  if (new Set(persistentFiles).size !== persistentFiles.length) throw new Error("Duplicate persistentFiles path");
  for (const replacement of replacesCoreFiles) {
    if (!targets.has(replacement)) throw new Error(`Core replacement must also be an overlay target: ${replacement}`);
  }
...
export function planComponentInstall(target, componentRoot, manifest, currentManifest) {
  ...
  const coreClaimed = collectCorePaths(current.core);
  const componentClaimed = new Set();
  const allowedCoreReplacements = new Set(normalized.replacesCoreFiles);
  ...
  for (const [id, component] of Object.entries(current.components)) {
    ...
    for (const path of collectOwnedPaths(component)) componentClaimed.add(path);
  }
  ...
  const entries = [
    ...
    if (componentClaimed.has(entry.target)) throw new Error(`Component file collision at ${entry.target}`);
    if (coreClaimed.has(entry.target) && !allowedCoreReplacements.has(entry.target)) {
      throw new Error(`Component file collision at ${entry.target}`);
    }
...
export function applyComponentInstall(target, componentRoot, manifest, currentManifest, options = {}) {
  ...
  try {
    ...
    const componentRecord = {
      ...
      replacesCoreFiles: [...plan.manifest.replacesCoreFiles],
      persistentFiles: [...plan.manifest.persistentFiles],
    };
