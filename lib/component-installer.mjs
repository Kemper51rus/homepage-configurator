...
export function validateComponentManifest(manifest, root) {
  ...
  if (typeof manifest.id !== "string" || !/^[a-z0-9](?:[a-z0-9._-]{0,62}[a-z0-9])?$/.test(manifest.id)) {
...
export function planComponentInstall(target, componentRoot, manifest, currentManifest) {
  ...
  const entries = [
    ...
    return {
      ...
      targetPath,
      existing,
      replaced: entry.kind === "owned" && existing,
    };
  });
...
export function applyComponentInstall(target, componentRoot, manifest, currentManifest, options = {}) {
  ...
  const backupRootRelative = `${backupDirectoryName}/component-${plan.manifest.id}-${uniqueSuffix()}`;
  ...
  try {
    ...
    for (const file of plan.files) {
      ...
      let backupPath = null;
      if (file.replaced) {
        backupPath = pathFromPosix(backupRoot, file.relativePath);
        mkdirSync(dirname(backupPath), { recursive: true });
        copyFileSync(file.targetPath, backupPath);
      }
      mutations.push({ targetPath: file.targetPath, existed: file.existing, backupPath });
    ...
    for (const mutation of mutations.reverse()) {
      if (mutation.existed && mutation.backupPath && existsSync(mutation.backupPath)) atomicCopy(mutation.backupPath, mutation.targetPath);
...
export function removeComponent(target, componentId, currentManifest, options) {
  ...
  const component = normalized.components[componentId];
