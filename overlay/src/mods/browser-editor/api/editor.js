...
const configuratorVersion = "0.7.0";
...
function scheduleHomepageRestart(status = null) {
  ...
  child.on("error", async (error) => {
    if (!status) {
      logger.error(`Не удалось запланировать перезапуск ${getServiceName()}: ${error.message}`);
      return;
    }
    const nextStatus = {
      ...status,
...
function getExactComponentOperationInput(body) {
  ...
  const unexpectedKey = Object.keys(body).find((key) => !componentOperationBodyKeys.has(key));
  if (unexpectedKey) {
