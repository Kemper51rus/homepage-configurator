...
- установщик;
- [`version.json`](../version.json) для браузерной проверки обновлений;
...
- версия в `package.json` и `version.json` должна совпадать; это проверяет `npm run check:version`.
...
- API редактирования включается только при `HOMEPAGE_BROWSER_EDITOR=true`. Дополнительный токен поверх внешней авторизации не используется.
